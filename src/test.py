...
class Rsp(serializers.Serializer):
    id = serializers.IDField('身份证', required=True)
    age = serializers.IntField('年龄', required=True)
...
@bp.post("/<age>/",
         ...
         response_serializer=serializers.ListSerializer(child=Rsp()),
         body_serializer=serializers.ListSerializer(child=Rep()))
async def index_create(request, age, *args, **kwargs):
    ...
    """
    return [{'content': 'this is index', 'name': '', 'age': age, 'id': 'sss'}]
